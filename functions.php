...
/* Show CPT in Blogroll */
function cpt_filter($query) {
	if( $query->is_main_query() && is_home() ) {
		$query->set('post_type', array( 'post', 'trail_runs' ) );
	}
}
add_action('pre_get_posts','cpt_filter');

/* Show CPT in Category & Tag archive pages */
...
/* Add Default Category to Trail Runs */
function save_trailruns_meta( $post_id, $post, $update ) {
    $slug = 'trail_runs'; //Slug of CPT
    if ( $slug != $post->post_type ) {
        return;
    }
	wp_set_object_terms( get_the_ID(), 'run-train', 'category' );
}
add_action( 'save_post', 'save_trailruns_meta', 10, 3 );

/* Add Default Category to Events */
function save_events_meta( $post_id, $post, $update ) {
    $slug = 'event'; //Slug of CPT
    if ( $slug != $post->post_type ) {
        return;
    }
	wp_set_object_terms( get_the_ID(), 'events', 'category' );
}
